...
DRUG_SCHEDULE_DB = {
    ...
    "cefixime":          {"schedule": "H1", "category": "Third gen antibiotic"},
    "ceftriaxone":       {"schedule": "H1", "category": "Third gen antibiotic"},
    "cefpodoxime":       {"schedule": "H1", "category": "Third gen antibiotic"},
    "cefuroxime":        {"schedule": "H1", "category": "Third gen antibiotic"},
    "cefoperazone":      {"schedule": "H1", "category": "Third gen antibiotic"},
    ...
    "phenobarbitone":    {"schedule": "H",  "category": "Antiepileptic / Sedative"},
    ...
    "antacid":           {"schedule": "OTC", "category": "Antacid"},
    ...
    "ors":               {"schedule": "OTC", "category": "Rehydration"},
    ...
    "droperidol":        {"schedule": "BANNED", "category": "Antipsychotic — banned"},
    ...
    "quinalbarbitone":   {"schedule": "BANNED", "category": "Barbiturate — banned in India"},
    "pemoline":          {"schedule": "BANNED", "category": "CNS stimulant — banned"},
    "nitrofurazone":     {"schedule": "BANNED", "category": "Topical antibiotic — banned"},
    "furazolidone":      {"schedule": "BANNED", "category": "Antibiotic — banned in India"},
    "nifuroxazide":      {"schedule": "BANNED", "category": "Antibiotic — banned"},
    "aristolochic acid": {"schedule": "BANNED", "category": "Herbal — banned due to nephrotoxicity"},
...
SCHEDULE_EXPLANATIONS = {
    "H": (
        "This drug is classified under Schedule H of the Drugs & Cosmetics Act. "
        "It can only be dispensed with a valid doctor's prescription. "
        "The label must display 'Schedule H' or the Rx symbol prominently."
    ),
    "H1": (
        "This drug is classified under Schedule H1 — a stricter category introduced "
        "to control antimicrobial resistance and high-risk medications. "
        "It requires a prescription and must be sold only against a written prescription "
        "retained by the pharmacist. Label must clearly state 'Schedule H1'."
    ),
    "X": (
        "This drug is classified under Schedule X — the highest control category "
        "covering psychotropics, narcotics, and strong sedatives. "
        "It requires a prescription, strict record keeping by the pharmacist, "
        "and the label must display 'Schedule X' prominently."
    ),
    "OTC": (
        "This drug is available over the counter and does not require a prescription. "
        "No Schedule H, H1, or X marking is required. "
        "However all other mandatory label requirements under Schedule M still apply."
    ),
    "BANNED": (
        "WARNING: This drug or formulation has been banned in India. "
        "Manufacturing, sale, and distribution is prohibited under the Drugs & Cosmetics Act. "
        "Immediate review by the regulatory affairs team is required."
    )
}


def lookup_drug(drug_name_text):
    """
    Takes the first line of a label (drug name) and tries to match
    it against the database. Returns schedule info or None.
    """
